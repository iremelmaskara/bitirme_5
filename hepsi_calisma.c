...
float y_onceki = 0;
float y, a_deger, b_deger ; 
float ayrik_zaman_denklem_cozum(float x);
float ADC_to_Voltage(unsigned int adc_deger);
unsigned int Voltage_to_DAC(float voltaj);
//***********PID katsayilari****
float Kp = 2.0;
float Ki = 0.1;
float Kd = 1.0;
//***********degiskenler********
float K = 0.98;      // Kazanç
float to = 0.050;    // Zaman sabiti
float T = 0.0080;   // Örnekleme süresi

//********************************************* Ayrik zamanli diferansiyel denklem çözümü (Euler ile)
//float ayrik_zaman_denklem_cozum(float x) 
//{
//    float a_deger = T*K/(to+T);
//    float b_deger = to/(to+T);
//    float y = a_deger*x+b_deger*y_onceki;
//    y_onceki = y;  // Hafizada tut
//    return y;
//}

//****************ADC ve DAC için voltaj dönüsüm fonksiyonlari*******
//float ADC_to_Voltage(unsigned int adc_deger) {
//    return (adc_deger * 5.0) / 4095.0;  // 12-bit ADC, 5V referans
//}

//unsigned int Voltage_to_DAC(float voltaj) {
//    if (voltaj < 0) voltaj = 0;
//    if (voltaj > 5) voltaj = 5;
//    return (unsigned int)((voltaj / 5.0) * 4095);
//}
//********************************
